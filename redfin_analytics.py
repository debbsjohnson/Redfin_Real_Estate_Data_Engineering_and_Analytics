...
# S3 bucket
target_bucket_name = 'redfin-transform-prj'
...
def transform_data (task_instance):
    ...
    cols = ['period_begin', 'period_end', 'period_duration', 'region_type', 'region_type_id', 'table_id', 'is_seasonally_adjusted', 'city', 'state', 'state_code', 'property_type', 'property_type_id', 'median_sale_price', 'median_list_price', 'median_ppsf', 'median_list_ppsf', 'homes_sold', 
    'inventory', 'months_of_supply', 'median_dom', 'avg_sale_to_list', 'sold_above_list', 'parent_metro_region_metro_code', 'last_updated'] 

    df = df[cols]
    df = df.dropna()

    # let's change the period begin and period end to date time object and extract years and month. 
    ...
    # mapping the month number to their respective month name
    month_dict = {
...
with DAG('redfin_analytics_dag',
        ...
        load_to_s3 = BashOperator (
            ...
            bash_command = 'aws s3 mv {{ ti.xcom_pull("tsk_extract_redfin_data")[0]}} s3://store-raw-data-prj'
        )
